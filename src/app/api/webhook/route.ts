...
export async function POST(req: NextRequest) {
    ...
    if (eventType === "call.session_started") {
        ...
        const [exisitingMeeting] = await db.select().from(meetings).where(
            and(
        ...
        const [exisitingAgent] = await db.select().from(agents).where(eq(agents.id, exisitingMeeting.agentID));
        if (!exisitingAgent) {
        ...
        const [exisitingMeeting] = await db.select().from(meetings).where(and(eq(meetings.id, channelId), eq(meetings.status, "completed")));
        if (!exisitingMeeting) {
        ...
        if (userId !== exisitingAgent.id) {

                const instructions = `
                You are an AI assistant helping the user revisit a recently completed meeting.
                Below is a summary of the meeting, generated from the transcript:
                
                ${exisitingMeeting.summary}
            ...
            ({
                role: message.user?.id === exisitingAgent.id ? "assistant" : "user",
            ...
            const GPTReponseText = GPTResponse.choices[0].message.content;
            if (!GPTReponseText) {
                return NextResponse.json(
